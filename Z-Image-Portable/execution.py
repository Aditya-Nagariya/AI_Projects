...
async def validate_prompt(prompt_id, prompt, partial_execution_list: Union[list[str], None]):
    ...
    for x in prompt:
        if 'class_type' not in prompt[x]:
            ...
            logging.error(f"Debug: Node {x} missing class_type. Data: {node_data}")
            return (False, error, [], {})

        class_type = prompt[x]['class_type']
        class_ = nodes.NODE_CLASS_MAPPINGS.get(class_type, None)
        # logging.info(f"Debug: Validating node {x} type '{class_type}'. Found class: {class_ is not None}. Output node: {getattr(class_, 'OUTPUT_NODE', False)}")
        if class_ is None:
            node_data = prompt[x]
